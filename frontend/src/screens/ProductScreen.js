...
const ProductScreen = () => {
  //get product details from product.js
  //const product = products.find((p) => p._id === match.id);

  const match = useParams(); //using useParams insted direct match
  ...
  */
  ...
  const navigate = useNavigate(); //in new version
      ...
      {loading ? (
        ...
        <Row>
          <Col md={6}>
            ...
            {/* fluid = to take image inside container */}
          </Col>
          <Col md={3}>
